...
export class OptimalHoldTargetSelector implements IOptimalHoldTargetSelector {
  ...
  constructor(payCalculator: PayCalculator) {
    ...
    hashGenerator.update(
      JSON.stringify(Object.getOwnPropertyNames(this.pay_calculator.pay_table))
    );
    ...
    // 2. Loop through all 2,598,960 combinations of 5 cards out of 52.
    for (let i = 0; i < allPossibleHandList.length; ++i) {
      if (i % progressUnit === 0) {
        process.stdout.write(
          `${((i / allPossibleHandList.length) * 100).toFixed(1)}%\r`
        );
      }
      const hand = allPossibleHandList[i];
      //Score it according to its poker value.
      const handPayTitle =
        this.pay_calculator.calculateHandResult(hand).pay_title;
      if (handPayTitle) {
        for (const possibleHandAfterDiscard of subsets(hand)) {
          ...
          switch (possibleHandAfterDiscard.length) {
            ...
            case 4:
              ...
              break;
            case 5:
              // Put the score in array0. Put the first hand in element 0 of the array
    ...
    try {
      fs.writeFileSync(
        fileCacheName,
        JSON.stringify(this.discard0ValueList) + "\n"
      );
      fs.appendFileSync(
        fileCacheName,
        JSON.stringify(this.discard1ValueMapList) + "\n"
      );
      fs.appendFileSync(
        fileCacheName,
        JSON.stringify(this.discard2ValueMapList) + "\n"
      );
      fs.appendFileSync(
        fileCacheName,
        JSON.stringify(this.discard3ValueMapList) + "\n"
      );
      fs.appendFileSync(
        fileCacheName,
        JSON.stringify(this.discard4ValueMapList) + "\n"
      );
      fs.appendFileSync(
        fileCacheName,
        JSON.stringify(this.discard5ValueMapList) + "\n"
      );
  ...
  private getExpectedOutcomeFromDiscardValueArray(
    holdingCardList: TCard[]
  ): Readonly<{ case_count: number; result: Readonly<IPayTitleMap<number>> }> {
    ...
    switch (holdingCardList.length) {
      ...
      default:
        throw new Error(
          `Invalid card list length: ${
            holdingCardList.length
  ...
  getPossibleOutcomeOfHoldingCards(
    ...
    const expectedOutcome =
      this.getExpectedOutcomeFromDiscardValueArray(holdingCardList);
    const expectedOutcomeWithDiscard: IExpectedOutcome = {
      case_count: expectedOutcome.case_count,
      result: Object.assign({}, expectedOutcome.result),
    };
    const discardedCardList = origCardList.filter(function (card) {
      return holdingCardList.indexOf(card) === -1;
    });
    for (const discardedCardListSubset of subsetsWithoutEmpty(
      discardedCardList
    )) {
      const impossibleHand = holdingCardList.concat(discardedCardListSubset);
      const impossibleHandResult =
        this.getExpectedOutcomeFromDiscardValueArray(impossibleHand);
  ...
  getPossibleOutcomeOfHoldingCardsWithoutDiscard(
    holdingCardList: TCard[]
  ): IExpectedOutcome {
    const expectedOutcome =
      this.getExpectedOutcomeFromDiscardValueArray(holdingCardList);
    const expectedOutcomeCopy: IExpectedOutcome = {
      case_count: expectedOutcome.case_count,
      result: Object.assign({}, expectedOutcome.result),
    };
  ...
  getExpectedValueWithPossibleHoldingHandList(
    ...
    const expectedValueWithPossibleHoldingHandList: {
      hand: TCard[];
      expected_value: number;
      outcome: IExpectedOutcome;
    }[] = [];
    for (const possibleHandAfterDiscard of subsets(cardList)) {
      const possibleOutcomeMap = this.getPossibleOutcomeOfHoldingCards(
        cardList,
        possibleHandAfterDiscard
      );
      let totalSumValue = 0;
      // console.log("possibleOutcomeMap.result", possibleOutcomeMap.result);
      for (const key in possibleOutcomeMap.result) {
        totalSumValue +=
          this.pay_calculator.pay_table[key] * possibleOutcomeMap.result[key];
      }
  ...
  selectHoldTargetIndices(cardList: TCard[]): number[] {
    const expectedValueWithPossibleHoldingHandList =
      this.getExpectedValueWithPossibleHoldingHandList(cardList);
    const highestExpectedValueWithHand: {
      hand: TCard[];
      expected_value: number;
    } = expectedValueWithPossibleHoldingHandList.reduce(function (acc, cur) {
    ...
    const highestExpectedValueHoldTargetIndexList: number[] =
      highestExpectedValueWithHand.hand.map(function (card) {
...
export class CheatedOptimalHoldTargetSelector implements IHoldTargetSelector {
  ...
  getExpectedValueForEachPossibleHoldingHandList(
    cardList: TCard[]
  ): { hand: TCard[]; expected_value: number }[] {
    const expectedValueWithPossibleHoldingHandList: {
      hand: TCard[];
      expected_value: number;
    }[] = [];
    for (const possibleHandAfterDiscard of subsets(cardList)) {
      const newCardList = this.game.next_card_list.slice(
        0,
        cardList.length - possibleHandAfterDiscard.length
      );
  ...
  selectHoldTargetIndices(): number[] {
    ...
    const expectedValueWithPossibleHoldingHandList =
      this.getExpectedValueForEachPossibleHoldingHandList(cardList);
    const highestExpectedValueWithHand: {
      hand: TCard[];
      expected_value: number;
    } = expectedValueWithPossibleHoldingHandList.reduce(function (acc, cur) {
